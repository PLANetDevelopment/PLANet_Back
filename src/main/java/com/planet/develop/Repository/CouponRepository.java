...
public interface CouponRepository extends JpaRepository<Coupon, String> {
    ...
    /** 쿠폰 개수 조회 */
    @Query("select count(c) from Coupon c where c.user.kakaoEmail = :id and c.availability = true")
    ...
    /** 쿠폰 조회 */
    @Query("select c.cno, c.coupon, c.remainingDays, c.discount, c.availability, c.expiration, cd.startDate, c.endDate, cd.usageInfo, cd.couponInfo, cd.detailInfo from Coupon c left join CouponDetail cd on c.cno = cd.cno where c.user.kakaoEmail = :id")
    List<Object[]> getCouponList(@Param("id") String id);

    /** 쿠폰 종료 날짜와 남은 날짜 조회 */
    @Query("select c.cno, c.remainingDays, c.endDate from Coupon c " +
            "where c.user.kakaoEmail = :id and c.availability = true and c.expiration = false")
